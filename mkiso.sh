...
_usage ()
{
    ...
    echo "    -DTB <DTB>         Set DTB for you board"
    echo "                        Default: ${dtb}"
...
make_boot() {
	...
	cp "${work_dir}/aarch64/airootfs/boot/dtbs/rockchip/${dtb}" "${work_dir}/iso/"
...
FDT /${dtb}
APPEND initrd=/arch/boot/aarch64/archiso.img console=tty1 archisobasedir=$install_dir archisolabel=${iso_label} video=eDP-1:1920x1080@60 slab_nomerge slub_debug=FZ mce=0 pti=on mds=full,nosmt iommu.strict=1 init_on_free=1 init_on_alloc=1 lockdown=confidentiality audit=1 audit_backlog_limit=8192 net.ifnames=0 modules.sig_enforce=1 oops=panic ipv6.disable=1
EOF
}
...
while getopts 'N:V:L:P:A:D:DTB:w:o:g:vh' arg; do
    case "${arg}" in
        N) iso_name="${OPTARG}" ;;
